...
2. v1.0.2のUnityPackageをImportします。
...
1. v1.0.2へ更新し、**Setup — 自動インストール / 更新** を1回実行します。
2. Setupの **Write Defaults互換モード** を **自動検出（推奨）** にします。
3. Modular Avatarを更新し、対象構成で必要な場合はMMD World Supportを有効にします。
4. Play Modeだけでなく、実際に使用するMMDワールドで表情・リップシンク・汗オン・Auto Modeを確認します。
...
Setupは、汗表現用Submeshが追加された生成Body Meshを割り当て、RealSweat Material Slotを追加する場合があります。復元情報は生成された `RealSweat` オブジェクトに保存されるため、このオブジェクトだけを削除すると、Modular AvatarのMenu、Parameters、FXは次のBuildから外れても、Bodyには生成MeshとMaterial Slotが残ることがあります。
...
`Assets/CookiTools/Generated` フォルダーの削除は別の問題です。削除したProjectフォルダーがCtrl+Zで復元できることを前提にせず、直後の復旧手順に従い、**導入前の正確なBody Meshを先に割り当て直してください**。

Undoを使用できない場合は、次の順序で復旧します。

1. 現在のアバターを先にバックアップし、`Assets/CookiTools/Generated` や残っている生成アセットを削除しないでください。
2. Bodyの **Skinned Mesh Renderer > Mesh** を、導入直前に使用していた正確なMeshへ戻します。導入前のBackupまたは元のアバターPrefabを使用し、可能であればMesh Propertyだけを **Revert** してください。
3. Bodyの **Materials** 配列と追加対象Rendererも、同じBackupを基準に導入前の参照・順番・Slot数へ復元します。
4. `RealSweat_BodyShellMesh` などの生成Meshが割り当てられた状態ではSetupを実行しないでください。導入直前の状態を確実に特定できない場合は作業を止め、Backupまたは未導入のアバターと比較して復旧してください。
